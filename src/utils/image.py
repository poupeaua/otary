...
class Image:
    ...
    def add_points(
        ...
        Args:
            ...
            colors_points (np.ndarray, optional): color for each color. Defaults to None.
            default_color (tuple, optional): default color for points. Defaults to (0, 0, 255).
        ...
        # as we are working with image lines needs to be in type int to define precise pixels
        points = np.asarray(points).astype(int)
        
        # define the color of the line to be default color in case not define in parameter
    ...
    def add_segments(
        ...
        Args:
            ...
            as_vectors (bool, optional): whether to display segments as vectors with a direction. 
                The second coordinate define the tip of the arrow. Defaults to False.
            colors_segments (np.ndarray, optional): color of each segment. Defaults to None.
            default_color (tuple, optional): tuple in BGR space. Defaults to (0, 0, 255).
            thickness (int, optional): display thickness of the segments. Defaults to 3.
        ...
        if len(segments) != len(colors_segments):
            raise RuntimeError(
                f"The number of segments ({len(segments)}) do not coincide with the number "
                f"of colors ({len(colors_segments)})"
            )
    ...
    def add_ocr_outputs(
        ...
        """Return a new image with the bounding boxes displayed from a list of OCR single output.
    ...
    def shift(self, shift: np.ndarray, mode: str="constant") -> Image:
        ...
        """
        im = scipy.ndimage.shift(input=self.asarray, shift=geo.Vector(shift).cv2_space_coords, mode=mode)
        return Image(im)
    ...
    def rotate(self, angle: float, is_degree: bool=True, reshape: bool=True):
        ...
        Args:
            ...
            is_degree (bool, optional): whether the angle is in degree or not. Defaults to True.
    ...
    def center_image_to_point(
        ...
        Args:
            ...
            mode (str, optional): scipy shift interpolation mode. Defaults to "constant".
    ...
    def center_image_to_segment(
        ...
        Args:
            ...
            mode (str, optional): scipy mode for the translation. Defaults to "constant".
    ...
    def crop_around_segment_horizontal(
        ...
        """Crop around a specific segment in the image. This is done in three specific steps:
        1) shift the image so that the middle of the segment is in the middle of the image
        2) rotate the image by the angle of the segment so that the segment becomes horizontal
        3) crop the image 
        ...
        Args:
            ...
            default_extra_width (int, optional): additional width for cropping. Defaults to 75.
        ...
        if width_crop_rect is None:
            ...
            width_crop_rect = geo_segment.length + default_extra_width
            
        # calculate the coordinates of the rectangle for cropping
        x0, y0 = int(im.center[1] - heigth_crop_rect / 2), int(im.center[0] - width_crop_rect / 2)
        x1, y1 = int(im.center[1] + heigth_crop_rect / 2), int(im.center[0] + width_crop_rect / 2)
