...
class Image:
    ...
    def __init__(self, image: np.ndarray) -> None:
        self.asarray = image.copy()
    
    @property
    def center(self):
        return (np.array([self.asarray.shape[1], self.asarray.shape[0]]) / 2).astype(int)
...
def crop_image_horizontal(image: np.ndarray, x0, y0, x1, y1):
    image = image.copy()
    img_cropped = image[x0:x1, y0:y1]
    anchor = np.array([y0, x0])
    return img_cropped, anchor
...
def crop_image_around_line_horizontal(
    ...
    _img, translation_vector = center_image_to_line(image=_img, line=line)
    
    if show_translation:
        center_pt_img = Image(_img).center
        middle_pt_line = geo.Segment(line).centroid
    ...
    if width_crop_rect is None:
        ...
        width_crop_rect = geo.Segment(line).length + extra_length_default_width
    ...
    _img_cropped, anchor = crop_image_horizontal(_img, x0, y0, x1, y1)

    _img_cropped_resized = resize_image(image=_img_cropped, scale_percent=resize_scale_percent)
    return _img_cropped_resized, anchor, translation_vector, angle_degree
